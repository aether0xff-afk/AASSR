...
강화학습에서는 특히 **temporal 보상 책임 배분**가 중요하다.
...
희소 보상에서는 보상 책임 배분 이전에 **성공 trajectory 자체를 발견하지 못하는 문제**가 있다.

행동 수가 각 단계마다 `b`, 성공까지 필요한 깊이가 `d`라고 단순화하면 random [탐색(exploration)](Exploration-and-Exploitation)으로 특정 경로를 고를 확률은 대략:
...
희소 보상 문제를 쉽게 만드는 대표적인 방법은 **보상 shaping**이다.
...
- 실제 observed 누적 보상 사용
- 다음 상태 가치 이어받기 bias가 없음
...
어떤 행동은 즉시 보상를 주지 않지만 정보를 얻어 미래의 행동 space를 바꿀 수 있다.
...
Random이 너무 쉽게 성공하면 sparse 탐색 연구가 의미가 없다.
