...
AASSR Imagination에서 가장 중요한 수학적 구분 중 하나는 **Chance node**와 **Decision node**다.
...
Decision node에서는 agent가 action을 선택할 수 있다.
...
Chance node에서는 이미 action을 선택했고, 그 뒤 환경 outcome이 확률적으로 갈린다.
...
이므로 expected-return 관점에서는 B가 더 좋다.
...
를 쓰면 agent가 실제로 통제할 수 없는 randomness를 선택할 수 있는 것처럼 취급한다.
...
두 action이 같은 expectation을 가져도 variance는 다를 수 있다.
...
AASSR current main planner는 기본 sparse-return expectation semantics를 유지한다.
...
그래서 AASSR Prophecy의 mixture outcome probability를 Imagination이 보존해야 한다.
...
Decision node에서 `max`를 쓴다고 해서 실제 Policy가 반드시 planner의 모든 future decision을 완벽히 실행한다는 뜻은 아니다.

Planner는 미래 state에서 available actions를 평가하고 최선의 continuation을 가정한다.

이것은 model-based lookahead의 기본 가정이다.

실제 environment에서는 첫 action만 실행하고 다시 관측해 planning하므로 future imagined decisions는 **counterfactual evaluation용**이다.
...
Chance branch가 terminal success/failure로 끝나면 continuation decision이 없다.
...
그래서 AASSR Prophecy는 terminal class를 decision-critical target으로 본다.
...
Planner에서 predicted truncation을 어떻게 평가할지는 experiment contract와 일관되어야 한다.

AASSR 외부 reward에서는 administrative truncation을 failure `-1`로 자동 바꾸지 않는다.
...
AASSR Skill Prophecy에서도 beam을 자를 때 retained mass를 다시 정규화한다.
...
AASSR Prophecy는 mixture branches와 outcome probabilities를 명시적으로 다루는 방향을 사용한다.

---

# 15. Decision branch pruning

Decision node에서는 모든 action을 확장하기 비싸므로 candidate ranking/beam/dedup을 사용할 수 있다.
...
Concrete action이 많지만 relationally 같은 구조라면 expensive value computation을 공유할 수 있다.
...
하지만 최종 실제 action은 concrete identity로 다시 bind한다.

이것은 action probability를 합치는 chance operation이 아니라 **decision compute deduplication**이다.
...
Depth limit에서 각 future state의 Critic value가 나온다고 하자.
...
Root action expected value:

```math
0.25(0.8)+0.75(-0.2)=0.05
```

Critic value가 큰 branch 하나만 고르면 안 된다.

---

# 18. Model reliability와 chance tree

Prediction reliability가 낮은 outcome distribution은 정확한 expectation 계산 자체가 의미 없을 수 있다.
...
- Policy root prediction reliability
