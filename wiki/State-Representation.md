...
- [Neural Networks & Optimization](Neural-Networks-and-Optimization) — [학습에 사용하는 특징(feature)](Terminology-Guide) vector, one-hot encoding, normalization
...
현재 v3는 기존 관계 기반 v2 [상태를 요약한 표현(descriptor)](State-Representation) 뒤에 **latest 공개된 HTTP 상태 코드 channel**을 추가한다.
...
이전 관계 기반 상태에서는 전체 [의미 기준(semantic)](State-Representation) structure는 비슷하게 표현하면서도 최근 응답의 `403/404/429` 같은 공개된 signal을 잃을 수 있었다.
...
v3는 latest 상태 코드를 명시적으로 보존해 이 [결과를 미리 보지 않는 비공개 평가(blind)](Ablation-Benchmarking-and-Reproducibility) spot을 줄인다.
...
즉 [세계 모델(world model)](Model-Based-RL-and-World-Models)이 사용하는 latent/학습 특징 표현과 계획기가 사용하는 행동/상태 [실험 규칙(protocol)](Ablation-Benchmarking-and-Reproducibility) 사이에 명시적인 decoder가 있다.
...
현재 v3 의미 기준 [평가 점수(score)](Terminology-Guide)는 개념적으로 다음 네 종류의 correctness를 함께 본다.
...
과거 진단 실험에서 전체 의미 기준 similarity가 높아도 상태 코드 error가 실제 decision quality를 망칠 수 있다는 [증거(evidence)](Evidence-Matrix)가 나왔기 때문이다.
...
[KnowledgeStore](Knowledge)는 그와 별도로 [정보의 출처 기록(provenance)](Knowledge)와 [인과적으로 공정한(causal)](Causality-Leakage-and-Evaluation) timing을 가진 explicit [한 번의 문제 풀이 구간(episode)](Terminology-Guide) context를 관리한다.
...
[Critic local support](Critic-Support-and-OOD)는 query 상태/행동이 [실제 환경에서 관측된(real)](Research-Jargon-Guide) 학습 분포 근처인지 판단해야 한다.

Raw 실제 개체를 구분하는 ID distance를 쓰면 학습 중 보지 못한 rename 자체를 [학습 분포 밖(OOD)](Critic-Support-and-OOD)로 잘못 볼 수 있다.
...
[Skill](Skills)은 성공한 실제 개체를 구분하는 trajectory를 관계 기반 행동 [재사용 가능한 틀(template)](Skills)로 저장한다.
...
[Curriculum](Curriculum-Learning) level이 올라가면 새로운 상태/행동 분포이 나타날 수 있다.

Relational 표현이 잘 설계되면 낮은 level에서 배운 구조를 higher level에 공유할 수 있다.

하지만 higher level에서 새로운 의사결정에 중요한 variable이 생기는데 상태 요약 표현가 이를 표현하지 못하면 상태 aliasing이 다시 발생할 수 있다.
...
- 의사결정에 중요한 공개된 channel을 상태 요약 표현에 보존
...
Count 학습 특징와 binary/공개된 [확률(probability)](Stochasticity-Uncertainty-and-Probability) 학습 특징의 scale이 크게 다르면 neural optimization에 영향을 줄 수 있다.
...
- 실제 개체를 구분하는 [ASEQ](ASEQ) false-positive rate
...
> **Relational 상태 v3는 숨겨진 정답을 추가하는 표현이 아니라, 공개된 관측에서 실제 개체를 구분하는 name의 불필요한 차이는 줄이되 의사결정에 중요한 공개된 상태 코드와 실행에 필요한 [실제 개체 구분(concrete identity)](State-Representation)는 별도 경로로 보존하는 전이 표현이다.**
