...
[강화학습](Reinforcement-Learning)에서 [에이전트(agent)](Reinforcement-Learning)는 행동하고 [보상(reward)](Sparse-Reward-and-Credit-Assignment)를 받는다.
...
실제 task failure라면 `-1`, 단순 rate-limit/[외부 제한 종료(truncation)](Replay-Buffer-and-Episode-Boundaries) 등은 `0`으로 구분한다.

즉 에이전트는 **“방금 보상가 0이었으니 이 행동은 쓸모없다”**라고 단순 판단할 수 없다.
...
AASSR [표준 비교 실험(benchmark)](Ablation-Benchmarking-and-Reproducibility)는 [부분 관측(POMDP)](MDP-and-POMDP)에 가깝다.
...
[에이전트(Agent)](Reinforcement-Learning)는 simulator가 내부적으로 알고 있는 정답 target, exact hidden audit pressure, future outcome을 직접 볼 수 없다.
...
그래서 current AASSR은 [전이(transfer)](Relational-Representation-and-Generalization)가 필요한 [Policy](Policy), [Prophecy](Prophecy), [Critic](Critic), [Skill](Skills)에서 [relational representation](Relational-Representation-and-Generalization)을 사용한다.
...
[AASSR의 ASEQ](ASEQ)는 실제 [상태 전이(transition)](MDP-and-POMDP):
...
특히 같은 [의미 기반 상태(semantic state)](State-Representation)에서 같은 행동을 했는데 다시 같은 상태라면:

```text
S → A → S
```

실제로 진전이 없는 [제자리 반복(self-loop)](ASEQ)일 수 있다.

이 패턴이 experience로 확인되면 같은 제자리 반복를 계속 반복하지 않도록 후보를 억제할 수 있다.
...
처럼 상태가 진행하면 같은 [행동(action)](Reinforcement-Learning) type의 반복을 막지 않는다.

즉 [ASEQ(실제 상태-행동-다음 상태 기록)](ASEQ)는 **“반복 행동 금지”가 아니라 “관측된 무진전 반복 억제”**다.

---

# 5. Policy: 지금 상태만 보고 기본 행동을 고른다

현재 [Policy](Policy)의 중심은 relational [DQN(딥 Q-네트워크)](Q-Learning-DQN-and-TD)이다.
...
[Q-value](Value-Functions-and-Bellman-Equation)는 현재 행동 이후의 장기 external [누적 보상(return)](Value-Functions-and-Bellman-Equation)을 근사한다.

AASSR은 별도의 [정보 가치 잔차(information residual)](Policy)도 관리한다.
...
Task 보상 자체를 정보 bonus로 바꾸지 않고, 행동 ranking에서 정보를 얻는 가치와 task value를 구분하려는 설계다.

[Policy(정책 모델)](Policy)만 사용한다면 여기서 기본 행동이 정해진다.
...
[Calibration](Calibration)은 [검증용 분리 데이터(holdout)](Calibration) real 상태 전이을 사용해 [Prophecy](Prophecy) prediction의 reliability를 평가한다.
...
왜 [상태 코드까지 고려하는(status-aware)](Calibration)인가?

과거에는 전체 relational state가 비슷하게 맞아도 `403/404/429` 같은 **decision-critical public status를 틀리는 문제**가 있었다.

따라서 단순 global similarity만으로 “[세계 모델(world model)](Model-Based-RL-and-World-Models)이 믿을 만하다”고 판단하지 않는다.

---

# 9. Imagination: 실제로 하기 전에 미래를 펼친다

[Imagination](Imagination)은 [Prophecy](Prophecy)를 여러 단계 이어 붙이는 [counterfactual planner](Counterfactual-Planning-and-Search)다.
...
다음 state에서 어떤 행동을 할지는 에이전트가 고를 수 있다.

```math
V_{decision}=\max_aV(S',a)
```

가장 좋은 행동 continuation을 선택할 수 있다.

이 둘을 섞으면 “10% jackpot outcome을 에이전트가 마음대로 선택할 수 있다”는 잘못된 planning이 된다.
...
[Critic](Critic)은 imagined state가 external sparse-누적 보상 관점에서 얼마나 좋은지 추정한다.

현재는 relational [GRU(게이트 순환 유닛)](GRU-and-Sequence-Models) 기반 discounted sparse-누적 보상 estimator다.
...
현재 imagined state/행동 주변에 실제 [Critic(미래 가치 평가기)](Critic) training evidence가 부족하면:
...
Support는 보상도 아니고 value bonus도 아니다.
...
AASSR이 상상 속에서 100개의 future를 계산해도 **현실에서 실행하는 것은 첫 [실제 실행 행동(concrete action)](State-Representation) 하나**다.
...
[Skill(성공 절차 재사용)](Skills)은 이미 발견한 성공 구조의 재사용에 가깝고, **새로운 해결 경로를 만드는 creativity 자체와는 별개**다.
...
- [현재 구조(current architecture)](Current-Status) contract
- reduced validation
- multi-[난수 시드(seed)](Ablation-Benchmarking-and-Reproducibility) 표준 비교 실험
- [최종 비공개 평가(final blind)](Ablation-Benchmarking-and-Reproducibility)
...
> **AASSR은 [희소 보상(sparse reward)](Sparse-Reward-and-Credit-Assignment) 환경에서 실제 경험을 relational/semantic 구조로 기억하고, stochastic 세계 모델로 여러 future를 예측하며, calibration과 local value support가 충분할 때만 [반사실적 계획(counterfactual planning)](Counterfactual-Planning-and-Search)으로 [Policy](Policy)의 첫 행동을 바꾸려는 강화학습 시스템이다.**
