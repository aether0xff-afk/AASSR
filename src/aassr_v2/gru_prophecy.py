...
class OnlineGRUProphecy:
    ...
    Real transitions train the GRU with one-step truncated backpropagation.
    Parallel universes carry separate ``GRUMemory`` objects. Symbolic facts and
    available actions are recovered from the nearest observed next-state
    template to the predicted numerical vector.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if min(
            ...
            raise ValueError(
                "sizes and replay_limit must be positive"
            )
        ...
        def matrix(
            ...
            return [
                [
                    self._random.uniform(-scale, scale)
    ...
    def _nearest_templates(
        ...
        scored = [
            (
                cosine_similarity(vector, template.vector),
                template,
            )
        ...
        scored.sort(
            key=lambda item: (
                -item[0],
                tuple(sorted(item[1].facts)),
            )
        )
        return scored[:limit]

    def _confidence(self, action: Action) -> float:
        seen = self._action_counts.get(action.signature, 0)
        experience = seen / (seen + 4.0)
        loss_quality = 1.0 / (
            1.0 + max(0.0, self._mean_loss)
        )
        return max(
            0.0,
            min(1.0, experience * loss_quality),
        )
    ...
    def predict_step(
        ...
        *,
        ...
        probabilities = _softmax(
            [score * 4.0 for score, _ in nearest]
        )
        scaled = [
        ...
        total = sum(scaled)
        if total <= 0.0:
            scaled = [1.0 / len(nearest)] * len(nearest)
        else:
        ...
        return ProphecyStep(predictions, next_memory)
    ...
    def _apply_vector_gradient(
        ...
        for index in range(len(vector)):
            vector[index] -= (
                self.learning_rate * _clip(gradient[index])
            )
    ...
    def learn(
        ...
        output, next_memory, cache = self._forward(
            x,
            self._train_memory,
        )
        ...
        loss = (
            sum(value * value for value in error)
            / self.state_size
        )
