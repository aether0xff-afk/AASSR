...
class ContextualPolicy:
    ...
    The policy never interprets action names and does not assume that one action
    signature has the same value in every state.
    """
        ...
        *,
        ...
        ranked = sorted(
            (
                ScoredAction(
                    action,
                    self.value(state, action) + deltas.get(action.signature, 0.0),
                )
    ...
    def select(
        ...
        *,
        ...
        for action in state.available_actions:
            ...
            bonus = exploration_bonus * sqrt(log(total + 2.0) / (entry.count + 1.0))
            scored.append(ScoredAction(action, self.value(state, action) + bonus))
        scored.sort(key=lambda item: (-item.score, item.action.signature))
        return scored[0].action

    def observe_return(self, state: StateSnapshot, action: Action, target: float) -> None:
        key = state_key(state)
        ...
        global_entry = self._global.setdefault(action.signature, RunningValue())
    ...
    def score(
        ...
        *,
        ...
        for item in self._items[-limit:]:
            predictions = prophecy.predict(item.before, item.action, samples=samples)
            values.append(
                prediction_similarity(
                    expected_prediction_vector(predictions), item.after.vector
                )
            )
...
class AutonomousLearningAgent:
    """Online agent that discovers transitions without demonstrations."""
    ...
    def epsilon(self, episode: int) -> float:
        fraction = min(1.0, max(0.0, episode / self.config.epsilon_decay_episodes))
    ...
    def select_action(
        ...
        *,
        ...
        action = self.policy.select(
            ...
            epsilon=0.0,
            exploration_bonus=self.config.exploration_bonus if explore else 0.0,
        )
    ...
    def observe(
        ...
        predictions_before = self.prophecy.predict(before, action, samples=1)
        prediction_score = prediction_similarity(
            expected_prediction_vector(predictions_before), after.vector
        )
        ...
        holdout_before = (
            self.holdout.score(
                self.prophecy, limit=self.config.holdout_evaluation_limit
            )
            if validate_now
            else 0.0
        )
        ...
        self.holdout.commit(HoldoutTransition(before, action, after), train=train)
