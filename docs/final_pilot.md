...
> **범위 주의:** 이 파일럿은 정답 경로 전이를 코드가 수집해 Prophecy에 사전학습시키는 모듈 진단이다. Imagination 깊이와 각 부품의 인과 방향은 확인할 수 있지만, 자율 탐색·자율 발견·인간 개입 최소화의 근거로 사용하지 않는다. 해당 주장은 `configs/autonomous_main.json`과 `docs/main_experiment_design.md`의 시범 없는 본 실험으로 검증한다.

이 설정은 이전 파일럿에서 발견한 측정·환경 설계 문제를 수정한 뒤 본 실험 직전에 돌리는 최종 진단이다.
