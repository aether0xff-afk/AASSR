from __future__ import annotations

import argparse
import json
from pathlib import Path

from aassr_v2.learned_prophecy_fix_experiment import (
    LEARNED_PROPHECY_CONDITIONS,
    run_learned_prophecy_fix,
)


def _parse_checkpoints(value: str) -> tuple[int, ...]:
    values = tuple(
        int(item.strip())
        for item in value.split(",")
        if item.strip()
    )
    if not values:
        raise argparse.ArgumentTypeError("at least one checkpoint is required")
    return values


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--condition",
        choices=LEARNED_PROPHECY_CONDITIONS,
        required=True,
    )
    parser.add_argument("--seed", type=int, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--train-episodes", type=int, default=2000)
    parser.add_argument("--train-map-count", type=int, default=128)
    parser.add_argument("--evaluation-episodes", type=int, default=100)
    parser.add_argument(
        "--checkpoints",
        type=_parse_checkpoints,
        default=(0, 100, 250, 500, 1000, 2000),
    )
    args = parser.parse_args()
    payload = run_learned_prophecy_fix(
        args.output,
        condition=args.condition,
        seed=args.seed,
        train_episodes=args.train_episodes,
        train_map_count=args.train_map_count,
        evaluation_episodes=args.evaluation_episodes,
        checkpoints=args.checkpoints,
    )
    print(json.dumps(payload["final"], indent=2, sort_keys=True))


if __name__ == "__main__":
    main()
