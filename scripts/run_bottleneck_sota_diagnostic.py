...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Isolate AASSR Policy, Prophecy, scoring and planning bottlenecks "
            "with neural-policy and oracle diagnostic conditions."
        )
    )
    parser.add_argument(
        "--condition",
        choices=tuple(item.name for item in CONDITIONS),
        required=True,
    )
...
def main() -> None:
    ...
    payload = run_bottleneck_condition(
        args.output,
